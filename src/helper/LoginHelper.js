        ...
        if (URLDetails !== null) {
            ...
            if (responseJson.output.success !== 0) {
                ...
                if (output.length > 1) {
                    ...
                    // single url
                    assignUrl(output[0].url, output[0].username, password, navigation, loginInstance, dispatch);
                } else {
                    //no urls
...
export const assignUrl = async (crmUrl, crmUsername, crmPassword, navigation, loginInstance, dispatch) => {
    try {
        ...
        doUserLogin(crmUsername, crmPassword, crmUrl, navigation, loginInstance, dispatch);
    } catch (error) {
        console.log(error);
        showNetworkError(loginInstance);
    }
...
export const doUserLogin = async (username, password, url, navigation, loginInstance, dispatch) => {
    try {
        let trimmedUrl = url.replace(/ /g, '').replace(/\/$/, '');
        trimmedUrl = (trimmedUrl.indexOf('://') === -1) ? 'https://' + trimmedUrl : trimmedUrl;
        if (url.includes('www.')) {
