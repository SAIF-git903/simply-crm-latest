...
export default function Updates({moduleName, recordId}) {
  ...
  const {fields, history, isLoading} = useSelector(state => state.updates);
  ...
  function renderIcon(icon) {
    return <Icon name={icon || 'clock'} size={22} color={'white'} />;
  }

  function renderDetail(rowData, sectionID, rowID) {
    const renderValuesView = values => {
      const renderValue = stringValue =>
        !stringValue || stringValue.length === 0 ? 'empty' : stringValue;

      if (!values || values.length === 0) return null;

      return values.map(x => (
        <View key={x.fieldName}>
          <Text style={styles.text}>
            <Text style={styles.fieldNameText}>{x.fieldName}</Text> updated
          </Text>
          <Text style={styles.text}>
            <Text style={styles.fieldNameText}>from</Text>{' '}
            {renderValue(x.previous)}
          </Text>
          <Text style={styles.text}>
            <Text style={styles.fieldNameText}>to</Text>{' '}
            {renderValue(x.current)}
          </Text>
        </View>
      ));
    };
    ...
    return (
      <View style={styles.card}>
        ...
        <View
          style={{
            paddingTop: rowData.values.length === 0 && !rowData.comment ? 0 : 5,
          }}>
          {rowData.comment ? (
            <View>
  ...
  function renderCircle(rowData, sectionID, rowID) {
    return (
      <View style={styles.circle}>
        <View style={styles.innerCircle}>{renderIcon(rowData.icon)}</View>
      </View>
    );
  }
  ...
  return <View style={styles.wrapper}>{renderTimeline()}</View>;
}
