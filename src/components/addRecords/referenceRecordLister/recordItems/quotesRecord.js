...
class QuotesRecord extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false
    ...
    render() {
        if (!this.state.loading) {
            return (
                <TouchableOpacity 
                onPress={() => { this.props.onRecordSelect(this.props.item.id, this.props.item.quoteLable, this.props.index); }}
                >
                <View 
                ...
                >
                    <Text numberOfLines={1} style={{ fontWeight: 'bold', marginLeft: 10, paddingRight: 10 }}>
                        {this.props.item.quoteLable}
                    </Text>
                    <Text numberOfLines={1} style={{ fontWeight: 'bold', marginLeft: 10, paddingRight: 10 }}>
                        {this.props.item.total}
                    </Text>
                    <Text numberOfLines={1} style={{ fontWeight: 'bold', marginLeft: 10, paddingRight: 10 }}>
                        {this.props.item.quoteStage}
                    </Text>
                </View>
                </TouchableOpacity>
            );
...
export default connect(null)(QuotesRecord);
