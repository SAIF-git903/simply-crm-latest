...
class PurchaseOrderRecord extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false
    ...
    render() {
        if (!this.state.loading) {
            return (
                <TouchableOpacity 
                onPress={() => { this.props.onRecordSelect(this.props.item.id, this.props.item.poLable, this.props.index); }}
                >
                <View 
...
export default connect(null)(PurchaseOrderRecord);
