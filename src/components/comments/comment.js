...
import {deleteComment, setReplyTo, setEdit} from '../../ducks/comments';
import ChildrenList from './childrenList';
import {processFile} from '../../helper/showImage';
import {
  CustomLayoutLinear,
  CustomLayoutSpring,
} from '../../helper/layoutAnimations';
...
export default function Comment(props) {
  const {item, index, crmTz, scrollToIndex} = props;
      ...
      <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
        {item.children.length !== 0 ? (
          <Button
            ...
            text={`${item.children.length} ${getRepliesText(
    ...
    return (
      <View style={{paddingTop: 5}}>
        ...
        <Text style={styles.editedText}>
          Comment modified{' '}
          {moment(moment.tz(item.modifiedtime, crmTz).format()).fromNow()}
        </Text>
      </View>
    );
  }
      ...
      <View style={{...styles.commentBox, opacity: isBeingDeleted ? 0.45 : 1}}>
        <View style={styles.commentUserData}>
          <View style={styles.userIconBackground}>
            <Icon name={'user'} solid size={28} color={'#CCCCCC'} />
          </View>
          <View>
        ...
        <View>{renderCommentButtons()}</View>
      </View>
    );
  }
  ...
  return (
    <View
      ...
      <ChildrenList parentComment={item} isChildRendered={isChildRendered} />
      {renderLoading()}
    </View>
  );
}
